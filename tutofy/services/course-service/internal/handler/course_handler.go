...
func (h *CourseHandler) GetAllCourses(ctx context.Context, req *coursepb.GetAllCoursesRequest) (*coursepb.CoursesList, error) {
	...
	for _, c := range courses { list = append(list, toProto(c)) }
...
func (h *CourseHandler) CompleteCourse(ctx context.Context, req *coursepb.CompleteCourseRequest) (*coursepb.CourseResponse, error) {
	callerID := middleware.UserIDFromContext(ctx)
	callerRole := middleware.RoleFromContext(ctx)
	course, err := h.svc.CompleteCourse(ctx, callerID, callerRole, req.GetCourseId())
	if err != nil {
		if errors.Is(err, service.ErrForbidden) { return nil, status.Error(codes.PermissionDenied, "forbidden") }
		if errors.Is(err, repository.ErrNotFound) { return nil, status.Error(codes.NotFound, "course not found") }
		return nil, status.Error(codes.Internal, err.Error())
...
func (h *CourseHandler) SearchCourses(ctx context.Context, req *coursepb.SearchCoursesRequest) (*coursepb.CoursesList, error) {
	limit := req.GetLimit()
	if limit <= 0 { limit = 50 }
	courses, err := h.svc.SearchCourses(ctx, req.GetTutorId(), req.GetTag(), req.GetCourseType(), req.GetMinPrice(), req.GetMaxPrice(), limit, req.GetOffset())
	if err != nil { return nil, status.Error(codes.Internal, err.Error()) }
	list := make([]*coursepb.CourseResponse, 0, len(courses))
	for _, c := range courses { list = append(list, toProto(c)) }
	return &coursepb.CoursesList{Courses: list}, nil
