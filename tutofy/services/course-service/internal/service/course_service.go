...
func (s *courseService) PublishCourse(ctx context.Context, callerID, callerRole, courseID string) (*model.Course, error) {
	if callerRole != "tutor" && callerRole != "admin" { return nil, ErrForbidden }
...
func (s *courseService) CompleteCourse(ctx context.Context, callerID, callerRole, courseID string) (*model.Course, error) {
	course, err := s.repo.GetCourseByID(ctx, courseID)
	if err != nil { return nil, err }
	if callerRole != "admin" && course.TutorID != callerID { return nil, ErrForbidden }
	c, err := s.repo.SetCourseStatus(ctx, courseID, "completed")
	if err == nil && s.rdb != nil {
		_ = s.rdb.Del(ctx, "course:"+courseID).Err()
	}
	return c, err
...
func (s *courseService) DeleteCourse(ctx context.Context, callerID, callerRole, courseID string) error {
	...
	if callerRole != "admin" && course.TutorID != callerID { return ErrForbidden }
