...
type PaymentRepository interface {
	...
	HasCompletedLessonPayment(ctx context.Context, userID, lessonID string) (bool, error)
}
...
func (r *postgresRepo) CreatePayment(ctx context.Context, p *model.Payment) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO payments (id, user_id, course_id, lesson_id, amount, status, created_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		p.ID, p.UserID, p.CourseID, p.LessonID, p.Amount, string(p.Status), p.CreatedAt,
	)
	return err
}
...
func (r *postgresRepo) GetByID(ctx context.Context, id string) (*model.Payment, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, user_id, course_id, COALESCE(lesson_id,''), amount, status, created_at FROM payments WHERE id = $1`, id,
	).Scan(&p.ID, &p.UserID, &p.CourseID, &p.LessonID, &p.Amount, &statusStr, &p.CreatedAt)
...
func (r *postgresRepo) GetByUserID(ctx context.Context, userID string, limit, offset int32) ([]*model.Payment, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, user_id, course_id, COALESCE(lesson_id,''), amount, status, created_at
	...
	for rows.Next() {
		p := &model.Payment{}
		var statusStr string
		if err := rows.Scan(&p.ID, &p.UserID, &p.CourseID, &p.LessonID, &p.Amount, &statusStr, &p.CreatedAt); err != nil {
			return nil, err
		}
		p.Status = model.PaymentStatus(statusStr)
...
func (r *postgresRepo) UpdateStatus(ctx context.Context, id string, status model.PaymentStatus) (*model.Payment, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`UPDATE payments SET status = $1 WHERE id = $2
		 RETURNING id, user_id, course_id, COALESCE(lesson_id,''), amount, status, created_at`,
		string(status), id,
	).Scan(&p.ID, &p.UserID, &p.CourseID, &p.LessonID, &p.Amount, &statusStr, &p.CreatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrNotFound
...
func (r *postgresRepo) HasCompletedLessonPayment(ctx context.Context, userID, lessonID string) (bool, error) {
	var exists bool
	err := r.db.QueryRowContext(ctx,
		`SELECT EXISTS(SELECT 1 FROM payments WHERE user_id = $1 AND lesson_id = $2 AND status = 'completed')`,
		userID, lessonID,
	).Scan(&exists)
	return exists, err
}
