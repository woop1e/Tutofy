...
const COURSE_COLORS = [
  { bar: '#4c6eff', bg: 'rgba(76,110,255,0.1)',   text: '#4c6eff' },
  ...
  { bar: '#22be70', bg: 'rgba(34,190,112,0.1)',   text: '#22be70' },
];

const ProgressRing = ({ value, size = 80, strokeWidth = 7, color = '#4c6eff' }) => {
...
const Progress = () => {
  ...
  const enriched = useMemo(() =>
    enrollments.map((enr, i) => {
      ...
      const progress        = pd?.percentage ?? enr.progress ?? 0;
      const lessonsCompleted = pd?.lessons_completed ?? 0;
      const totalLessons    = pd?.total_lessons ?? (course.lessons_count ?? 0);
      return { ...enr, course, progress, lessonsCompleted, totalLessons, colorIdx: i % COURSE_COLORS.length };
    }),
  ...
  const stats = [
    { icon: <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5"><path d="M4 3h12a1 1 0 011 1v12a1 1 0 01-1 1H4a1 1 0 01-1-1V4a1 1 0 011-1z"/><path d="M8 3v14M4 7h4M4 11h4" strokeLinecap="round"/></svg>, label: 'Courses enrolled',    value: enriched.length,      color: '#4c6eff' },
    ...
    { icon: <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5"><circle cx="10" cy="10" r="8"/><path d="M6 10l3 3 5-5" strokeLinecap="round" strokeLinejoin="round"/></svg>, label: 'Courses completed',   value: completedCount,        color: '#22be70' },
    ...
    <div className="flex min-h-screen bg-[#f3f4f7] font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white h-[64px] border-b border-[#f0f0f5] flex items-center px-7 justify-between flex-shrink-0">
          <div>
            <p className="text-[#181b26] text-[17px] font-bold">My Progress</p>
            <p className="text-[#8a90a1] text-[12px]">Track your overall learning journey</p>
          </div>
        ...
        <div className="flex-1 p-6">
            ...
            {stats.map(stat => (
              <div key={stat.label} className="bg-white rounded-[14px] border border-[#f0f0f5] p-5 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[#8a90a1] text-[12px]">{stat.label}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-[16px] border border-[#f0f0f5] p-12 text-center">
              ...
              <p className="text-[#181b26] text-[17px] font-semibold mb-2">No progress yet</p>
              <p className="text-[#8a90a1] text-[14px] mb-6">Enroll in a course or book a private lesson to start tracking</p>
              <Link to="/tutors"
                className="bg-[#4c6eff] text-white text-[14px] font-semibold px-6 py-3 rounded-[10px] hover:opacity-90 transition-opacity">
                Find a Tutor
              </Link>
            </div>
          ) : (
              ...
              {enriched.length > 0 && (
                <div className="grid grid-cols-3 gap-6">
                  ...
                  <div className="bg-white rounded-[16px] border border-[#f0f0f5] p-6 flex flex-col items-center justify-center">
                    <p className="text-[#181b26] text-[15px] font-semibold mb-5">Course Progress</p>
                    <div className="relative w-20 h-20 mb-4">
                      <ProgressRing value={avgProgress} color="#4c6eff" />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-[#181b26] text-[16px] font-bold">{avgProgress}%</span>
                      </div>
                    </div>
                    <p className="text-[#8a90a1] text-[13px]">Across {enriched.length} course{enriched.length !== 1 ? 's' : ''}</p>
                    <div className="mt-5 w-full space-y-2">
                      <div className="flex justify-between text-[12px]">
                        <span className="text-[#8a90a1]">Completed</span>
                        <span className="text-[#22be70] font-semibold">{completedCount}</span>
                      </div>
                      <div className="flex justify-between text-[12px]">
                        <span className="text-[#8a90a1]">In Progress</span>
                        <span className="text-[#4c6eff] font-semibold">{inProgressCount}</span>
                      </div>
                      <div className="flex justify-between text-[12px]">
                        <span className="text-[#8a90a1]">Not Started</span>
                        <span className="text-[#8a90a1] font-semibold">{enriched.length - completedCount - inProgressCount}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2 bg-white rounded-[16px] border border-[#f0f0f5] p-5">
                    <div className="flex items-center justify-between mb-5">
                      <h2 className="text-[#181b26] text-[15px] font-semibold">Course breakdown</h2>
                      <Link to="/student/courses" className="text-[#4c6eff] text-[12px] font-medium hover:underline">View courses →</Link>
                    </div>
                          ...
                          <div key={enr.id} className="flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-1.5">
                                <p className="text-[#181b26] text-[13px] font-semibold truncate mr-2">
                                  {enr.course?.title || `Course ${enr.colorIdx + 1}`}
                                </p>
                                <span className="text-[12px] font-bold flex-shrink-0" style={{ color: c.text }}>{enr.progress}%</span>
                              </div>
                              <div className="w-full bg-[#f3f4f7] rounded-full h-[6px]">
                                <div className="h-[6px] rounded-full transition-all"
                                  style={{ width: `${Math.min(100, enr.progress)}%`, backgroundColor: c.bar }} />
                              </div>
                              {enr.totalLessons > 0 && (
                                <p className="text-[#8a90a1] text-[11px] mt-1">
                            ...
                            <span className="text-[11px] font-medium px-2.5 py-1 rounded-full flex-shrink-0"
                              style={
                                enr.progress >= 100
                                  ? { backgroundColor: 'rgba(34,190,112,0.15)', color: '#22be70' }
                                  : enr.progress > 0
                                  ? { backgroundColor: 'rgba(76,110,255,0.1)',   color: '#4c6eff' }
                                  : { backgroundColor: '#f8f9fc',                color: '#8a90a1' }
                              }>
              ...
              {privateLessons.length > 0 && (
                <div className="bg-white rounded-[16px] border border-[#f0f0f5] p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-[#181b26] text-[15px] font-semibold">Private lessons</h2>
                    <Link to="/student/courses" className="text-[#4c6eff] text-[12px] font-medium hover:underline">View all →</Link>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    <div className="px-4 py-3 rounded-[10px] border border-[#f0f0f5] bg-[#f8f9fc]">
                      <p className="text-[22px] font-bold text-[#935bf5] leading-none">{privateLessons.length}</p>
                      <p className="text-[#8a90a1] text-[12px] mt-1">Sessions booked</p>
                    </div>
                    <div className="px-4 py-3 rounded-[10px] border border-[#f0f0f5] bg-[#f8f9fc]">
                      <p className="text-[22px] font-bold text-[#22be70] leading-none">{privateCompleted}</p>
                      <p className="text-[#8a90a1] text-[12px] mt-1">Completed</p>
                    </div>
                    <div className="px-4 py-3 rounded-[10px] border border-[#f0f0f5] bg-[#f8f9fc]">
                      ...
                      <p className="text-[#8a90a1] text-[12px] mt-1">Upcoming</p>
                    </div>
                  </div>
