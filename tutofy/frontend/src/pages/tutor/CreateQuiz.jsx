...
/* ── helpers ── */
const inputCls = 'w-full border border-[#e2e4ea] rounded-[9px] px-3 py-2.5 text-[14px] text-[#181b26] placeholder-[#b0b5c4] focus:outline-none focus:border-[#4c6eff] focus:ring-2 focus:ring-[rgba(76,110,255,0.08)] transition-all bg-white';
...
const QuestionEditor = ({ q, idx, onChange, onDelete }) => {
  ...
  return (
    <div className="border border-[#4c6eff]/30 rounded-[12px] bg-[#fafbff] p-5">
      {/* Question header */}
      <div className="flex items-center gap-3 mb-4">
        <span className="w-7 h-7 rounded-full bg-[#4c6eff] text-white text-[12px] font-bold flex items-center justify-center flex-shrink-0">
          {idx + 1}
        </span>
        <span className="text-[11px] font-semibold text-[#4c6eff] bg-[#eef0ff] px-2.5 py-1 rounded-full">
          {typeMeta?.label}
        </span>
        <div className="flex items-center gap-1.5 ml-auto">
          <label className="text-[11px] text-[#8a90a1]">Points</label>
          <input
            type="number"
            min="1"
            value={q.points}
            onChange={(e) => update({ points: parseInt(e.target.value) || 10 })}
            className="w-16 border border-[#e2e4ea] rounded-[7px] px-2 py-1 text-[13px] text-[#181b26] focus:outline-none focus:border-[#4c6eff] text-center"
          />
          <button
            onClick={onDelete}
            className="ml-2 text-[#8a90a1] hover:text-[#f24545] transition-colors"
      ...
      {(q.type === 'multiple_choice' || q.type === 'multiple_response') && (
        <div className="space-y-2">
          {q.options.map((opt, oi) => (
            <div key={oi} className="flex items-center gap-2">
              <button
                ...
                className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${opt.isCorrect ? 'border-[#22be70] bg-[#22be70]' : 'border-[#d2d4d9] hover:border-[#4c6eff]'
                  }`}
              >
                {opt.isCorrect && (
              ...
              <input
                ...
                className="flex-1 border border-[#e2e4ea] rounded-[8px] px-3 py-2 text-[13px] text-[#181b26] placeholder-[#b0b5c4] focus:outline-none focus:border-[#4c6eff] bg-white"
              />
              {q.options.length > 2 && (
          ...
          <button
            onClick={addOption}
            className="flex items-center gap-1.5 text-[#4c6eff] text-[12px] font-medium mt-1 hover:opacity-80"
          >
          ...
          {q.options.map((opt, oi) => (
            <button
              ...
              className={`flex-1 py-2.5 rounded-[9px] text-[14px] font-semibold border-2 transition-colors ${opt.isCorrect
                ...
                : 'border-[#e2e4ea] text-[#8a90a1] hover:border-[#4c6eff]'
                }`}
            >
              {opt.text}
            </button>
      ...
      {(q.type === 'fill_blank' || q.type === 'short_answer') && (
        <div>
          <label className="block text-[11px] text-[#8a90a1] font-medium mb-1.5">
      ...
      {q.type === 'matching' && (
        <div>
          <div className="grid grid-cols-2 gap-2 mb-2">
            <span className="text-[11px] text-[#8a90a1] font-medium pl-1">Term</span>
            <span className="text-[11px] text-[#8a90a1] font-medium pl-1">Definition</span>
          </div>
          <div className="space-y-2">
            {q.options.map((opt, oi) => (
              <div key={oi} className="grid grid-cols-2 gap-2 items-center">
                <input
                  ...
                  className="border border-[#e2e4ea] rounded-[8px] px-3 py-2 text-[13px] focus:outline-none focus:border-[#4c6eff] bg-white"
                />
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 border border-[#e2e4ea] rounded-[8px] px-3 py-2 text-[13px] focus:outline-none focus:border-[#4c6eff] bg-white"
                  />
                  {q.options.length > 2 && (
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-[#4c6eff] text-[12px] font-medium mt-2 hover:opacity-80"
          >
            <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.6" className="w-3 h-3">
              <path d="M6 2v8M2 6h8" strokeLinecap="round" />
            </svg>
      ...
      {q.type === 'ordering' && (
        <div>
          <p className="text-[11px] text-[#8a90a1] font-medium mb-2">Items in correct order (top = first)</p>
          <div className="space-y-2">
            {q.options.map((opt, oi) => (
              <div key={oi} className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-[#f0f0f5] text-[#8a90a1] text-[11px] font-bold flex items-center justify-center flex-shrink-0">
                  {oi + 1}
                </span>
                <input
                  value={opt.text}
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="bg-white border-b border-[#ebebf0] h-[60px] flex items-center px-6 gap-4 flex-shrink-0">
          <Link
            ...
            className="flex items-center gap-1.5 text-[#8a90a1] text-[13px] hover:text-[#181b26] transition-colors flex-shrink-0"
          >
          ...
          <h1 className="text-[#181b26] text-[16px] font-bold">Create Quiz</h1>
          <span className="text-[11px] font-semibold text-[#8a90a1] bg-[#f0f0f5] px-2.5 py-1 rounded-full">Draft</span>

          <div className="ml-auto flex items-center gap-2">
            <button className="border border-[#d2d4d9] text-[#4c5162] text-[13px] font-semibold px-4 py-2 rounded-[9px] hover:border-[#4c6eff] hover:text-[#4c6eff] transition-colors">
              Preview quiz
            </button>
            <button
              onClick={() => handleSave(false)}
              disabled={saving}
              className="border border-[#d2d4d9] text-[#4c5162] text-[13px] font-semibold px-4 py-2 rounded-[9px] hover:border-[#4c6eff] hover:text-[#4c6eff] transition-colors disabled:opacity-50"
            >
              Save draft
            </button>
            <button
              ...
              disabled={saving}
              className="bg-[#4c6eff] text-white text-[13px] font-semibold px-4 py-2 rounded-[9px] hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center gap-2"
            >
        ...
        <div className="flex-1 flex gap-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-[700px] space-y-5">
              ...
              <div className="bg-white rounded-[16px] border border-[#ebebf0] p-6">
                <h2 className="text-[#181b26] text-[15px] font-bold mb-4">Quiz information</h2>

                <div className="mb-4">
                  <label className="block text-[12px] text-[#8a90a1] font-medium mb-1.5">Quiz title</label>
                  <input
                    value={info.title}
                ...
                <div className="mb-4">
                  <label className="block text-[12px] text-[#8a90a1] font-medium mb-1.5">
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div>
                    <label className="block text-[12px] text-[#8a90a1] font-medium mb-1.5">Time limit (minutes)</label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div>
                    <label className="block text-[12px] text-[#8a90a1] font-medium mb-1.5">Total points</label>
                    <input
                      type="number"
                      readOnly
                  ...
                  <div>
                    <label className="block text-[12px] text-[#8a90a1] font-medium mb-1.5">Passing score (%)</label>
                    <input
                      type="number"
                      min="0"
                      max="100"
              ...
              <div className="bg-white rounded-[16px] border border-[#ebebf0] p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-[#181b26] text-[15px] font-bold flex items-center gap-2">
                    Questions
                    {questions.length > 0 && (
                      <span className="w-6 h-6 rounded-full bg-[#4c6eff] text-white text-[11px] font-bold flex items-center justify-center">
                        {questions.length}
                      </span>
                    )}
                  </h2>
                ...
                {questions.length === 0 ? (
                  <div className="text-center py-10">
                    <div className="w-12 h-12 rounded-full bg-[#f0f2ff] flex items-center justify-center mx-auto mb-3">
                      <svg viewBox="0 0 20 20" fill="none" stroke="#4c6eff" strokeWidth="1.5" className="w-6 h-6">
                        <path d="M10 4v12M4 10h12" strokeLinecap="round" />
                      </svg>
                    </div>
                    <p className="text-[#8a90a1] text-[13px]">No questions yet. Choose a type from the right panel.</p>
                  </div>
                ) : (
                ...
                {questions.length > 0 && (
                  <div className="mt-5 flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-[#4c6eff] text-white text-[13px] font-semibold px-4 py-2.5 rounded-[10px] hover:opacity-90 transition-opacity shadow-[0px_4px_12px_0px_rgba(76,110,255,0.25)]"
                    >
                    ...
                    <div className="relative group">
                      <button className="flex items-center gap-1.5 text-[#8a90a1] text-[12px] font-medium px-3 py-2.5 rounded-[10px] border border-[#e2e4ea] hover:border-[#4c6eff] hover:text-[#4c6eff] transition-colors">
                        Different type
                        ...
                        {QUESTION_TYPES.filter(t => t.type !== lastType).map(qt => (
                          ...
                          >
                            <div className="w-7 h-7 rounded-[7px] bg-[#f0f2ff] flex items-center justify-center flex-shrink-0">
                              <svg viewBox="0 0 20 20" fill="none" stroke="#4c6eff" strokeWidth="1.4" className="w-3.5 h-3.5">
                                <path d={qt.icon} strokeLinecap="round" strokeLinejoin="round" />
                              </svg>
                            </div>
                            <div>
                              <p className="text-[#181b26] text-[12px] font-semibold">{qt.label}</p>
                              <p className="text-[#8a90a1] text-[11px]">{qt.sub}</p>
                            </div>
                          </button>
                        ))}
                      </div>
          ...
          {/* ── Right: question type panel ── */}
          <div
            id="qtype-panel"
            className="w-[260px] flex-shrink-0 border-l border-[#ebebf0] bg-white overflow-y-auto"
          >
            <div className="p-5">
              <h3 className="text-[#181b26] text-[14px] font-bold mb-4">Add question</h3>
              <div className="space-y-2">
                {QUESTION_TYPES.map((qt) => (
                  <button
                    key={qt.type}
                    onClick={() => addQuestion(qt.type)}
                    className="w-full flex items-center gap-3 p-3 rounded-[10px] border border-[#ebebf0] hover:border-[#4c6eff]/40 hover:bg-[#f8f9ff] transition-colors text-left group"
                  >
                    <div className="w-9 h-9 rounded-[8px] bg-[#f0f2ff] flex items-center justify-center flex-shrink-0 group-hover:bg-[#4c6eff]/15 transition-colors">
                      <svg viewBox="0 0 20 20" fill="none" stroke="#4c6eff" strokeWidth="1.4" className="w-4.5 h-4.5">
                        <path d={qt.icon} strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
                    <div className="min-w-0">
                      <p className="text-[#181b26] text-[13px] font-semibold leading-tight">{qt.label}</p>
                      <p className="text-[#8a90a1] text-[11px] mt-0.5">{qt.sub}</p>
                    </div>
                  </button>
                ))}
              </div>
